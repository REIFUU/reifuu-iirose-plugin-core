...
{
    const faceHolder = document.getElementById("faceHolder") as HTMLElement;
    // if (faceHolder === null) { return; }
    const faceHolderType = faceHolder.querySelector(".faceHolderType");
    const pageList = faceHolder.querySelector("div:nth-child(2)") as Element;
    // if (pageList === null) { return; }
    const initAddPageItem: Record<string, any> = {};
    ...
    {
        // @ts-ignore
        const isPage: Element = mutationsList[0].addedNodes[0];
        const className = isPage?.className;
        if (className === "emojiContentBox")
        {
            const index = isPage.getAttribute("index") as string;
            // if (!index) { return; }
     ...
     */
    function addPageItem(index: number, title: string, content: Element | string)
    {
        // @ts-ignore
     ...
     * @param {*} page 
     */
    function initCreatePage(index: string, page: any)
    {
        const emojiPage = page.querySelector(".emojiPage") as Element;
        const emojiContent = page.querySelector(".emojiContent") as Element;
        const emojiPageItem = emojiPage.querySelectorAll("span") as NodeListOf<HTMLSpanElement>;

        // 获取当前主题emojiPageItem选中颜色
        let electStyle = (emojiPage.querySelector("span[style]") as Element).getAttribute("style") as string;

        // emojiPageItem切换
        emojiPage.addEventListener("click", (event) =>
        {
            if ((event.target as Element).classList.contains("faceHolderPageItem"))
            {
                emojiPage.querySelectorAll("span").forEach(item =>
                {
                    item.setAttribute("style", "");
                });
                (event.target as Element).setAttribute("style", electStyle);

                emojiContent.querySelectorAll(":scope > div").forEach(item =>
                {
                    const targetP = (event.target as Element).getAttribute("p") as string;
                    const nowIndex = item.getAttribute("index") as string;
                    if (targetP == nowIndex)
                    {
        ...
        // 添加emojiPage
        initAddPageItem[index].forEach((item: any[], index: string) =>
        {
            // item
     ...
     */
    function createPage(index: number, page: any, title: string, content: Element)
    {
     ...
     * @returns 
     */
    function hiddenPageType(eq: number)
    {
        const /**@type {HTMLElement} */ targetType: HTMLElement = (faceHolderType as HTMLElement).querySelector(`span[eq="${eq}"]`) as HTMLElement;
        if (targetType)
        {
     ...
     * @returns 
     */
    function hiddenPageItem(index: number, p: number)
    {
        const /**@type {HTMLElement} */ targetPage: HTMLElement = pageList.querySelector(`div[index="${index}"]`) as HTMLElement;
        if (targetPage)
        {
            const /**@type {HTMLElement} */ targetItem: HTMLElement = targetPage.querySelector(`.emojiPage>span[p="${p}"]`) as HTMLElement;
            if (targetItem)
            {
...
export const createConfigPage = (() =>
{
    ...
    const plugConfigPageArr: Record<string, any> = {};
    ...
    // devtest
    function newElement(str: string)
    {
     ...
     */
    function addPage(plugin: any, Element: Element)
    {
     ...
     * @param {string} target
     */
    function switchConfigPage(target: string)
    {
        const controls = plugConfigBox.querySelector('div') as HTMLDivElement;
        const nowIndex = controls.getAttribute("index") as string;
        if (nowIndex != target)
        {
     ...
     * @returns 
     */
    function createPlugContent(plugin: REIFUU_Plugin)
    {

        // 读取localStorage内插件的配置
        const key = `reifuuTemp.${plugin.name}`;
        const readData: string = (localStorage.getItem(key)) ? localStorage.getItem(key) as string : "{}";
     ...
     * @returns {Element}
     */
    function createTipsElement(text: string, type: number)
    {
    ...
    function createConfigElement(plugin: any)
    {
    ...
    function createConfigItem(schema: Schema, title: string, pluginConfig: Record<string, any>)
    {
        ...
        {
            description.textContent = schema.meta.description as string;
        }
        ...
        switch (schema.type)
        {
            case "number": {
                ...
                plus.addEventListener("click", () =>
                {
                    try { input.value = schema(parseInt(input.value, 10) + 1); } catch (err)
                    {
                        ...
                        {
                            input.value = String(schema.meta.max);
                        } else
                        {
                            input.value = "0";
                        }
                ...
                sub.addEventListener("click", () =>
                {
                    try { input.value = schema(parseInt(input.value, 10) - 1); } catch (err)
                    {
                        ...
                        {
                            input.value = String(schema.meta.min);
                        } else
                        {
                            input.value = "0";
                        }
                    ...
                    if (window.isMobile)
                    {
                        ...
                        Utils.sync(2, ["请输入值", null, null, input.value], (e) =>
                        {
                            if (e !== null)
                            {
                                try
                                {
                                    const value = (schema(isNaN(parseInt(e, 10)) ? schema.meta.min : parseInt(e, 10)));
                                    input.value = value;
                                }
                                catch (err)
                                {
                                    ...
                                    {
                                        input.value = schema();
                                    } else if (schema.meta.hasOwnProperty('min') && ((parseInt(e, 10) < (schema.meta.min as number))))
                                    {
                                        input.value = String(schema.meta.min);
                                    } else if (schema.meta.hasOwnProperty('max') && ((parseInt(e, 10) > (schema.meta.max as number))))
                                    {
                                        input.value = String(schema.meta.max);
                                    }
                                    else
            ...
            case "button": {
                ...
                {
                    pluginConfig[(schema as any).click]();
                });
            ...
            case "array": {
                ...
                if (value.length <= 0)
                {
                    createArrItem(schema.type, null, null);
                } else
                {
                 ...
                 * @returns 
                 */
                function createArrItem(type: any, value: any, index: number | null)
                {
                    ...
                    input.addEventListener("click", () =>
                    {
                        ...
                        if (window.isMobile)
                        {
                            ...
                            {
                                if ((schema as any).inner.type === "number")
                                {
                                    arr[index as number] = parseInt(e, 10);
                                } else
                                {
                                    arr[index as number] = e;
                                }
                                updateArr(arr);
                    ...
                    up.addEventListener("click", () =>
                    {
                        ...
                        {
                            [nowArr[index as number], nowArr[index as number - 1]] = [nowArr[index as number - 1], nowArr[index as number]];
                        }
                        updateArr(nowArr);
                    });
                    ...
                    down.addEventListener("click", () =>
                    {
                        ...
                        if ((index as number) < nowArr.length - 1)
                        {
                            [nowArr[index as number], nowArr[index as number + 1]] = [nowArr[index as number + 1], nowArr[index as number]];
                        }
                        updateArr(nowArr);
